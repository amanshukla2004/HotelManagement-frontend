...
const StepBar = ({ current }) => (
  <div className="flex items-center justify-between px-2 mb-10">
    {STEPS.map((label, i) => (
      ...
      >
        <div
          className={`relative z-10 w-8 h-8 rounded-2xl flex items-center justify-center text-xs font-black transition-all duration-500 border ${
            i < current
              ? "bg-[#F97316] text-white border-transparent shadow-lg shadow-[#F97316]/30"
              : i === current
        ...
        {i < STEPS.length - 1 && (
          <div
            className={`absolute left-1/2 top-4 w-full h-[1px] -z-0 transition-all duration-700 ${
...
const inputClass =
  "w-full bg-gray-50 border border-transparent rounded-2xl px-4 py-3 text-sm font-bold text-[#0F172A] focus:bg-white focus:border-[#F97316] focus:ring-4 focus:ring-[#F97316]/5 outline-none transition-all placeholder:text-gray-300";
const labelClass =
...
const BookingDialog = ({ hotel, room, onClose }) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error?.message ||
          "Booking failed. Please check your dates.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateGuest = async () => {
    ...
    setAddingGuest(true);
    try {
      const res = await guestApi.createGuest({
        ...newGuest,
        age: Number(newGuest.age),
      });
      ...
      >
        ...
        >
          ...
          <div className="px-8 pt-8 pb-4 flex items-start justify-between">
            <div>
              <h2 className="text-3xl font-black text-[#0F172A] tracking-tighter uppercase leading-none mb-2">
                Book Your Stay
              </h2>
            ...
            <button
              onClick={onClose}
              className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 hover:text-[#0F172A] transition-colors"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="px-8 pb-10">
            ...
            {step === 0 && (
              <form
                onSubmit={handleSubmit(handleInitBooking)}
                className="space-y-6"
              >
                <div className="bg-gray-50 rounded-[2rem] p-6 border border-gray-100 flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <h4 className="text-xl font-black text-[#0F172A] tracking-tighter uppercase">
                      {room?.type}
                    </h4>
                    <p className="text-xs font-black text-[#F97316] uppercase tracking-widest">
                      {hotel?.name}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div
                    ...
                    onClick={() =>
                      document.getElementById("dialog-in").showPicker()
                    }
                  >
                  ...
                  <div
                    className="cursor-pointer"
                    onClick={() =>
                      document.getElementById("dialog-out").showPicker()
                    }
                  >
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        {...register("checkOutDate", {
                          ...
                          validate: (value) =>
                            new Date(value) >=
                              new Date(checkinValue || todayStr()) ||
                            "Invalid date",
                        })}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 bg-gray-50 rounded-2xl p-4">
                    <span className="text-xs font-black text-gray-500 uppercase tracking-[0.2em] flex-1">
                      Number of Units
                    </span>
                    <div className="flex items-center gap-4">
                      ...
                      <input
                        ...
                        max={room?.totalCount}
                        {...register("roomsCount")}
                        className="w-16 bg-white border border-gray-100 rounded-xl text-center py-2 font-black text-[#0F172A]"
                      />
                    </div>
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-5 bg-[#0F172A] hover:bg-[#F97316] text-white rounded-[1.5rem] font-black text-xs uppercase tracking-[0.3em] transition-all shadow-xl shadow-[#0F172A]/20 flex items-center justify-center gap-3 active:scale-[0.98]"
                >
                  {loading ? (
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between px-1">
                  ...
                  {selectedGuestIds.length > 0 && (
                    <span className="text-xs font-black text-[#F97316] bg-[#F97316]/5 px-2 py-0.5 rounded-md uppercase tracking-widest">
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-4 p-4 rounded-2xl border transition-all ${
                          selectedGuestIds.includes(g.id)
                            ? "border-[#F97316] bg-[#F97316]/5 ring-2 ring-[#F97316]/10"
                            : "border-gray-50 bg-white hover:border-gray-200 shadow-sm"
                        }`}
                      >
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black shadow-sm ${selectedGuestIds.includes(g.id) ? "bg-[#F97316] text-white" : "bg-gray-100 text-gray-400"}`}
                        >
                ...
                {showAddGuest ? (
                  ...
                  >
                    <p className="text-xs font-black text-[#F97316] uppercase tracking-widest mb-4">
                      Add Guest
                    </p>
                    <input
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <select
                        ...
                        onChange={(e) =>
                          setNewGuest((p) => ({ ...p, gender: e.target.value }))
                        }
                      ...
                      <input
                        ...
                        value={newGuest.age}
                        onChange={(e) =>
                          setNewGuest((p) => ({ ...p, age: e.target.value }))
                        }
                        className={inputClass}
                      />
                    </div>
                    <div className="flex gap-2 pt-2">
                      <button
                        ...
                        className="flex-1 py-3 text-xs font-black text-gray-400 uppercase tracking-widest hover:bg-white rounded-xl transition"
                      >
                        Dismiss
                      </button>
                      <button
                        ...
                        className="flex-1 py-3 bg-[#F97316] text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-[#F97316]/20"
                      >
                  ...
                  <button
                    ...
                    className="w-full py-4 border border-dashed border-gray-200 text-gray-400 hover:text-[#F97316] hover:border-[#F97316] hover:bg-[#F97316]/5 rounded-2xl flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest transition-all"
                  >
                ...
                <div className="flex gap-4 pt-4">
                  <button
                    onClick={() => setStep(2)}
                    className="flex-none px-8 py-4 border border-gray-100 text-gray-400 hover:text-[#0F172A] hover:bg-gray-50 rounded-2xl text-xs font-black uppercase tracking-widest transition-all"
                  >
                    Skip
                  </button>
                  <button
                    ...
                    className="flex-1 py-4 bg-[#0F172A] hover:bg-[#F97316] text-white rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl shadow-[#0F172A]/20 disabled:opacity-30"
                  >
                    Add Guest & Continue
                  </button>
                </div>
            ...
            {step === 2 && (
              <div className="space-y-8">
                <div className="relative group">
                  ...
                  <div className="relative bg-white border border-gray-100 rounded-[2.5rem] p-8">
                    <div className="flex justify-between items-start mb-8 relative z-10">
                      <div>
                        <p className="text-xs font-black text-[#F97316] uppercase tracking-[0.3em] mb-1">
                          Reservation Details
                        </p>
                        <h4 className="text-3xl font-black text-[#0F172A] tracking-tighter leading-none">
                          {room?.type}
                        </h4>
                    ...
                    <div className="space-y-4 mb-10">
                      ...
                      <div className="flex justify-between items-center pt-2">
                        ...
                        <span className="text-3xl font-black text-[#F97316] tracking-tighter shrink-0 ml-2">
                          ₹
                    ...
                    <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100/50 flex items-start gap-3">
                      ...
                      />
                      <p className="text-xs font-bold text-amber-700 leading-relaxed uppercase tracking-widest">
                        Test Environment: This is a demo payment system. No real
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full py-5 bg-[#F97316] hover:bg-[#0F172A] text-white rounded-[1.5rem] font-black text-xs uppercase tracking-[0.3em] transition-all shadow-xl shadow-[#F97316]/30 flex items-center justify-center gap-3 active:scale-95"
                >
                  {loading ? (
                    <Loader2 size={18} className="animate-spin" />
                  ) : (
              ...
              <div className="text-center py-6">
                {bookingStatus === "CONFIRMED" ? (
                  ...
                  >
                    ...
                    <h3 className="text-4xl font-black text-[#0F172A] tracking-tighter uppercase leading-none mb-4">
                      Booking Confirmed
                    </h3>
                    <p className="text-sm font-medium text-gray-400 max-w-sm mx-auto mb-10">
                      Your booking is successful! You can view your details in
                      your profile.
                    </p>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-10">
                      <div className="p-4 bg-gray-50 rounded-3xl text-left border border-gray-100">
                        ...
                        <p className="text-xs font-black text-[#0F172A]">
                          #ROOMLY-{String(bookingId).slice(-6).toUpperCase()}
                        </p>
                      </div>
                      <div className="p-4 bg-gray-50 rounded-3xl text-left border border-gray-100">
                        <p className="text-xs font-black text-gray-400 uppercase mb-1">
                        ...
                        <p className="text-xs font-black text-[#0F172A]">
                          {selectedGuestIds.length} Companion(s)
                        </p>
                      </div>
                    </div>
                    ...
                    <button
                      onClick={onClose}
                      className="w-full py-5 bg-[#0F172A] hover:bg-[#F97316] text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all"
                    >
                      Go Back
                    </button>
                  </motion.div>
                  ...
                  <div className="space-y-8">
                    ...
                    <div>
                      <h3 className="text-3xl font-black text-[#0F172A] tracking-tighter uppercase leading-none mb-2">
                        Verifying Payment...
                      </h3>
                      <p className="text-sm font-medium text-gray-400 max-w-sm mx-auto">
                        Waiting for payment confirmation. This may take up to 60
                        seconds.
                      </p>
                    </div>
                    ...
                    <div className="bg-amber-50 border border-amber-100 p-6 rounded-[2rem] text-left">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="px-2 py-0.5 bg-amber-500 text-white text-xs font-black rounded uppercase">
                          {bookingStatus?.replace(/_/g, " ") || "CHECKING"}
                        </span>
                      </div>
                      <p className="text-xs font-bold text-amber-600 leading-relaxed tracking-wider uppercase">
                        Your status will update automatically once the bank
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <button
                        onClick={onClose}
                        className="w-full py-4 text-gray-400 hover:text-[#0F172A] font-black text-xs uppercase tracking-widest"
                      >
