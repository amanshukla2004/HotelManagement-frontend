...
const LoginPage = () => {
    ...
    try {
      ...
      if (isRegistering) {
        if (roleMode === "GUEST") {
          ...
          const loginRes = await authApi.userLogin({
            email: data.email,
            password: data.password,
          });
          loginUser(
            loginRes.data.accessToken || loginRes.data.data?.accessToken,
            "GUEST",
          );
        } else {
          await authApi.managerRegister({
            name: data.name,
            email: data.email,
            password: data.password,
          });
          const loginRes = await authApi.managerLogin({
            email: data.email,
            password: data.password,
          });
          loginUser(
        ...
        if (roleMode === "GUEST") {
          ...
          response = await authApi.managerLogin({
            email: data.email,
            password: data.password,
          });
          loginUser(
  ...
  const inputClass =
    "w-full bg-white/50 border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-sm font-semibold text-gray-800 outline-none focus:border-[#F97316] focus:ring-4 focus:ring-[#F97316]/10 transition-all placeholder:text-gray-400";
  const labelClass =
    "block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1 px-1";
      ...
      >
        ...
        <div className="hidden md:flex md:w-1/2 relative bg-gray-100 overflow-hidden items-end justify-center p-8">
          ...
          <div ref={textRef} className="relative z-10 text-white w-full">
            ...
            <h1 className="text-4xl font-black mb-2 leading-tight font-display">
              Discover the
              <br />
              <span className="text-[#F97316]">World's Best</span>
            </h1>
            <p className="text-sm text-gray-200 font-medium max-w-sm">
              Join millions of travelers exploring luxury stays and premium
              resorts globally.
            </p>
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 p-8 md:p-10 flex flex-col justify-center bg-white">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 mb-4">
              ...
              <span className="text-xl font-black text-[#0F172A] tracking-tight uppercase font-display">
                Roomly Manager
              </span>
            </div>
          ...
          <div className="flex p-1 bg-gray-100/80 rounded-2xl border border-gray-200 mb-6">
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[11px] font-bold uppercase tracking-wider transition-all ${
                roleMode === "HOTEL_MANAGER"
                  ? "bg-[#0284C7] shadow-md text-white"
                  : "text-gray-500 hover:text-gray-700"
              }`}
            >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <button
              ...
              className={`w-full py-3.5 rounded-xl text-white font-black text-[11px] uppercase tracking-widest transition-all mt-6 flex justify-center items-center gap-2 shadow-lg ${
                roleMode === "HOTEL_MANAGER"
                  ? "bg-[#0284C7] hover:bg-[#0369A1] shadow-[#0284C7]/20"
                  : "bg-[#F97316] hover:bg-[#EA580C] shadow-[#F97316]/20"
          ...
          <div className="mt-6 text-center">
            ...
            >
              {isRegistering ? (
                ...
                <span className="flex items-center justify-center gap-1.5">
                  New here? <span className="text-[#0284C7]">Sign Up</span>{" "}
                  <ArrowRight size={14} />
                </span>
              )}
            </button>
