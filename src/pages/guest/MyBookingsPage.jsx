...
const MyBookingsPage = () => {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error?.message || "Failed to load your bookings.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-6xl mx-auto px-4 pt-32 pb-20 space-y-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <span className="px-3 py-1 bg-[#0284C7]/10 text-[#0284C7] text-xs font-black uppercase tracking-widest rounded-md">
                My Account
              </span>
            ...
            <p className="text-sm font-medium text-gray-400 max-w-lg">
              View and manage your current bookings, past stays, and upcoming
              trips.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <RefreshCw
                size={20}
                className={loading && !bookings.length ? "animate-spin" : ""}
              />
            </button>
            <button
              onClick={() => navigate("/")}
        ...
        <div className="space-y-8">
          {loading && !bookings.length ? (
            ...
            <div className="bg-white rounded-[3rem] border border-gray-100 p-24 text-center shadow-lg relative overflow-hidden group">
              ...
              <h3 className="text-2xl font-black text-[#0F172A] tracking-tighter uppercase mb-4">
                No Bookings Found
              </h3>
              <p className="text-sm font-medium text-gray-400 max-w-xs mx-auto mb-10 leading-relaxed">
                You haven't made any bookings yet. Start your journey by finding
                a perfect hotel.
              </p>
              <button
                onClick={() => navigate("/")}
                className="px-10 py-5 bg-[#0F172A] text-white rounded-[1.5rem] font-black text-xs uppercase tracking-widest hover:bg-[#0284C7] transition-all active:scale-95 shadow-2xl shadow-[#0F172A]/20"
              >
            ...
            <div className="space-y-6">
              <AnimatePresence>
                {bookings.map((booking, idx) => {
                  ...
                  const amount = Number(
                    booking.amount ?? booking.totalPrice ?? 0,
                  );
                  const isPending =
                    booking.bookingStatus === "PAYMENTS_PENDING";
                      ...
                      <div className="p-8 md:p-10 flex flex-col md:flex-row items-center gap-10">
                        ...
                        <div className="relative shrink-0">
                          <div className="w-32 h-32 rounded-[2rem] bg-gray-50 flex items-center justify-center relative border border-gray-100 overflow-hidden group-hover:scale-105 transition-transform duration-500">
                            ...
                            <div
                              className={`absolute inset-0 ${config.bg} opacity-10`}
                            />
                          </div>
                          <div
                            className={`absolute -bottom-2 -right-2 p-2.5 rounded-xl border bg-white shadow-xl ${config.color} ${config.border}`}
                          >
                            {config.icon}
                          </div>
                        </div>
                        ...
                        <div className="flex-1 space-y-5 text-center md:text-left">
                          <div className="space-y-1">
                            <div className="flex items-center justify-center md:justify-start gap-4 mb-2">
                              <span className="text-xs font-black text-[#0284C7] uppercase tracking-widest bg-[#0284C7]/5 px-3 py-1 rounded-md">
                                ID: #{booking.id}
                              </span>
                              <div
                                className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest border ${config.bg} ${config.color} ${config.border}`}
                              >
                                {config.label}
                              </div>
                            </div>
                          ...
                          <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 pt-2 border-t border-gray-50 pt-5">
                            ...
                            <div className="text-center md:text-left">
                              ...
                              <p className="text-xs font-black text-[#0F172A] uppercase">
                                {booking.roomsCount || 1} Room
                                {(booking.roomsCount || 1) > 1 ? "s" : ""}
                              </p>
                            </div>
                            <div className="text-center md:text-left">
                              <p className="text-xs font-black text-gray-300 uppercase tracking-widest mb-1">
                        ...
                        <div className="flex flex-col gap-3 w-full md:w-auto">
                          {(booking.bookingStatus === "PAYMENTS_PENDING" ||
                            booking.bookingStatus === "GUESTS_ADDED" ||
                            ...
                            <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl">
                              ...
                              <p className="text-[10px] font-bold text-amber-600/80 leading-tight uppercase tracking-wider">
                                Session expired. Please rebook this hotel to
                                complete your stay.
                              </p>
                            </div>
                          )}
                            ...
                            booking.bookingStatus !== "EXPIRED" && (
                              <button
                                ...
                                disabled={
                                  ...
                                  !isCancelable(booking.bookingStatus)
                                }
                                className={`flex items-center justify-center gap-3 px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all border active:scale-95 ${
                                  isCancelable(booking.bookingStatus)
        ...
        {selectedBooking &&
          createPortal(
            <div className="fixed inset-0 w-full h-full z-[9999] flex items-center justify-center p-6 sm:p-12 overflow-hidden text-left">
              <div
                className="absolute inset-0 bg-[#0F172A]/40 backdrop-blur-[20px]"
              ...
              <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-[0_60px_150px_-20px_rgba(0,0,0,0.5)] overflow-hidden">
                ...
                <div className="p-10 md:p-12 relative z-10">
                  <div className="flex justify-between items-start mb-10">
                    <div>
                      <p className="text-[10px] font-black text-[#0284C7] uppercase tracking-[0.3em] mb-2">
                        Authenticated Stay
                      </p>
                      <h3 className="text-4xl font-black text-[#0F172A] tracking-tighter uppercase leading-none">
                        Stay Details
                      </h3>
                    </div>
                    <button
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <div className="space-y-8">
                      <div>
                        ...
                        <h4 className="text-2xl font-black text-[#0F172A] tracking-tighter uppercase leading-none">
                          {selectedBooking.hotel?.name ||
                            selectedBooking.hotelName ||
                            "Roomly Elite Stay"}
                        </h4>
                        <p className="text-xs font-bold text-gray-400 mt-3 flex items-center gap-2 uppercase tracking-widest">
                          <MapPin size={12} className="text-[#0284C7]" />{" "}
                          {selectedBooking.hotel?.city || selectedBooking.city}
                        </p>
                      </div>
                      <div className="space-y-4 p-6 bg-gray-50 rounded-[2.5rem] border border-gray-100">
                        ...
                        <div className="flex justify-between border-t border-gray-200 pt-4">
                          ...
                          <span className="text-xl font-black text-[#0284C7] tracking-tighter">
                            ₹{Number(selectedBooking.amount).toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="space-y-3 max-h-56 overflow-y-auto pr-2 custom-scrollbar">
                        {selectedBooking.guests?.map((g, i) => (
                          <div
                            key={i}
                            className="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl border border-gray-100 group/guest hover:bg-gray-100 transition-colors"
                          >
                            <div className="w-10 h-10 rounded-xl bg-gray-200 flex items-center justify-center text-[10px] font-black text-gray-600">
                  ...
                  <div className="mt-12">
                    <button
                      onClick={() => setSelectedBooking(null)}
                      className="w-full py-5 bg-[#0284C7] text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-[#0F172A] transition-all shadow-xl shadow-[#0284C7]/20"
                    >
                      Return to Dashboard
                    </button>
                  </div>
        ...
        <div className="bg-[#0F172A] rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl flex flex-col md:flex-row items-center justify-between gap-10">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-1">
              <p className="text-xs font-black text-[#0284C7] uppercase tracking-[0.3em]">
                Security
              </p>
              <p className="text-xl font-bold tracking-tight">
                Your booking history is encrypted and secure.
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            <button
              ...
              className="px-8 py-4 bg-white/5 border border-white/10 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-white/10 transition-all"
            >
              Support Center
            </button>
          </div>
