  ...
  return (
    ...
    >
      ...
      <div className="flex-1 space-y-4 text-center md:text-left">
        <div>
          <h4 className="text-2xl font-black text-[#0F172A] tracking-tighter uppercase leading-none mb-1 group-hover:text-[#0284C7] transition-colors">
            {room.type}
          </h4>
          <div className="flex items-center justify-center md:justify-start gap-4 text-xs font-black text-gray-400 uppercase tracking-widest">
            <span className="flex items-center gap-1.5">
              <Users size={12} className="text-[#0284C7]" /> Guests:{" "}
              {room.capacity}
            </span>
        ...
        <div className="flex flex-wrap justify-center md:justify-start gap-2">
          {(room.amenities || []).map((a) => (
            <span
              key={a}
              className="px-2 py-1 bg-gray-50 text-xs font-black text-gray-400 uppercase tracking-widest rounded-md border border-gray-100"
            >
              {a}
            </span>
          ))}
      ...
      <div className="text-center md:text-right shrink-0 md:pl-10 md:border-l border-gray-50">
        <p className="text-xs font-black text-gray-300 uppercase tracking-[0.2em] mb-1">
          Price
        </p>
        <h5 className="text-3xl font-black text-[#0F172A] tracking-tight mb-4 font-display">
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setError(
          err.response?.data?.error?.message || "Failed to load hotel details.",
        );
      } finally {
        setLoading(false);
      }
  ...
  if (error)
    return (
      <div className="flex items-center justify-center h-[80vh] flex-col gap-6 text-red-400 max-w-sm mx-auto text-center px-4">
        ...
        <div>
          <h3 className="text-xl font-black text-[#0F172A] uppercase tracking-tighter mb-2">
            Loading Error
          </h3>
        ...
        <button
          onClick={() => navigate(-1)}
          className="px-8 py-3.5 bg-[#0F172A] text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-[#0F172A]/20 active:scale-95 transition-all"
        >
          ← Go Back
        </button>
      </div>
    ...
    <div className="bg-[#F8FAFC] min-h-screen">
      ...
      <section className="relative w-full h-[70vh] md:h-[85vh] overflow-hidden bg-[#0F172A]">
        ...
        <div className="absolute top-20 md:top-24 left-4 md:left-8 z-50">
          <button
            onClick={() => navigate(-1)}
            className="group flex items-center gap-2 bg-white/20 backdrop-blur-xl border border-white/30 px-5 py-2.5 rounded-xl text-white transition-all hover:bg-white hover:text-[#0284C7] shadow-xl"
          >
            <ChevronLeft
              size={16}
              className="group-hover:-translate-x-1 transition-transform"
            />
            <span className="text-[11px] font-bold uppercase tracking-wider leading-none pt-0.5">
              All Hotels
            </span>
          </button>
        </div>
        ...
        <div className="absolute left-4 right-4 bottom-32 md:left-12 md:bottom-40 pointer-events-none z-40">
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              <span className="px-3 py-1 bg-[#F97316] text-white text-[10px] font-bold uppercase tracking-wider rounded-md">
                Premium Hotel
              </span>
            ...
            <h1 className="text-4xl md:text-6xl font-black text-white tracking-tight leading-none mb-3 max-w-3xl drop-shadow-2xl font-display">
              {hotel?.name}
            </h1>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                {
                  label: "Status",
                  val: hotel?.active ? "Active" : "Private",
                  icon: <UserCheck size={16} />,
                },
              ].map((stat) => (
                <div
                  key={stat.label}
                  className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex flex-col items-center text-center group hover:bg-[#0284C7] transition-all duration-500"
                >
                  ...
                  <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1 group-hover:text-white/60 transition-colors">
                    {stat.label}
                  </p>
                  <p className="text-xs font-black text-[#0F172A] uppercase tracking-tighter group-hover:text-white transition-colors">
                    {stat.val}
                  </p>
                </div>
              ))}
            ...
            <div className="bg-white rounded-[2.5rem] border border-gray-100 p-8 md:p-12 shadow-sm">
              <h3 className="text-xs font-black text-[#0284C7] uppercase tracking-[0.3em] mb-4">
                About this Hotel
              </h3>
              <p className="text-gray-500 leading-relaxed font-medium mb-10 text-lg">
                Experience the pinnacle of hospitality at {hotel?.name}. Each
                room is designed with high-end architecture and modern amenities
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                {(hotel?.amenities || []).map((a) => (
                  <div key={a} className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-black text-[#0F172A] uppercase tracking-widest whitespace-nowrap">
                      {a}
                    </span>
                  </div>
                ))}
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6">
                {rooms.length === 0 && (
                  <p className="p-12 text-center text-gray-400 bg-white rounded-[2rem] border border-gray-100">
                    Rooms details are being updated.
                  </p>
                )}
                {rooms.map((room) => (
                  <RoomCard
          ...
          <aside className="space-y-8 sticky top-32">
            ...
            <div className="bg-white p-4 rounded-[2.5rem] border border-gray-100 shadow-xl overflow-hidden group">
              ...
              <div className="p-4 pt-6">
                <p className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-2 px-1">
                  Precise Location
                </p>
                <p className="text-sm font-bold text-[#0F172A] leading-snug">
                  {hotel?.contactInfo?.address || "Restricted Address"},{" "}
                  {hotel?.city}
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#0F172A] rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl">
              ...
              <h4 className="text-xs font-black text-[#0284C7] uppercase tracking-[0.3em] mb-8">
                Help & Support
              </h4>
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-4 group cursor-pointer">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold break-all pr-2">
                      {hotel?.contactInfo?.email || "support@roomlystays.com"}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-16 space-y-8">
          ...
          <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
            {photos?.map((photo, i) => (
              <div
                key={i}
                className="break-inside-avoid rounded-[2.5rem] overflow-hidden bg-gray-50 border border-gray-100 shadow-sm group relative"
              >
                <img
                  src={photo}
                  alt={`${hotel?.name} ${i}`}
