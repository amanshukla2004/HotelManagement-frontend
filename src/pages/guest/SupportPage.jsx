...
const SupportPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const templateParams = {
      ...
      message: "EMAIL FOR NOX ADMINS " + "EMAIL: " + formData.email + " | TYPE: " + formData.type + " | MESSAGE: " + formData.message,
    };
    ...
    try {
      ...
      setStatus({
        type: 'error',
        message: err.text || 'Failed to send message. Please check your credentials or try again.'
      });
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-[#F8FAFC] pt-32 pb-20">
      <div className="max-w-4xl mx-auto px-4 space-y-12">
        ...
        <div className="text-center space-y-4">
          ...
          <h1 className="text-5xl font-black text-[#0F172A] tracking-tighter uppercase leading-none">How can we help?</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                ...
                <AnimatePresence>
                  {status && (
                    <motion.div
                      ...
                      className={`flex items-center gap-3 p-4 rounded-2xl text-xs font-bold ${status.type === 'success' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'
                        }`}
                    >
          ...
          <div className="space-y-6">
            <div className="bg-[#0F172A] p-6 rounded-[4rem] text-white overflow-hidden relative shadow-xl">
              ...
              <p className="text-[20px] font-black text-[#0284C7] uppercase tracking-[0.3em] mb-4">Direct Contact</p>
              <h3 className="text-xl font-black tracking-tight uppercase mb-6">Support Recipient</h3>
              <div
                onClick={() => {
                  navigator.clipboard.writeText(supportEmail || 'work.amanshukla2004@gmail.com');
                  setCopied(true);
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-black text-white/40 uppercase tracking-widest">
                    {copied ? <span className="text-green-400">Copied to clipboard!</span> : 'Email Address'}
                  </p>
                  <p className="text-sm font-black text-white break-all">{supportEmail || 'Loading...'}</p>
                </div>
              </div>
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
              <h4 className="text-xs font-black text-[#0F172A] uppercase tracking-widest mb-6">Developer Status</h4>
              <div className="space-y-4">
        ...
        <div className="space-y-8">
          <div className="flex items-center gap-3 px-1">
            ...
            <h2 className="text-xl font-black text-[#0F172A] uppercase tracking-tighter">Frequent Questions</h2>
          </div>
              ...
              >
                <div className="flex justify-between items-start gap-4">
                  <div className="space-y-2">
                    <h4 className="text-sm font-black text-[#0F172A] uppercase tracking-tight group-hover:text-[#0284C7] transition-colors">{faq.q}</h4>
                    <p className="text-sm font-medium text-gray-400 leading-relaxed">{faq.a}</p>
                  </div>
                  <ChevronRight size={18} className="text-gray-200 group-hover:text-[#0284C7] group-hover:translate-x-1 transition-all" />
                </div>
              </motion.div>
            ))}
          </div>
