...
const HotelSummaryPage = () => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center">
        <RefreshCw className="animate-spin text-[#635BFF]" size={40} />
      </div>
    );
  }
  ...
  if (!hotel) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center space-y-6">
        ...
        <h2 className="text-2xl font-black text-[#0A2540] uppercase tracking-tighter">Hotel Not Found</h2>
        <button onClick={() => navigate('/admin/hotels')} className="px-8 py-4 bg-[#0A2540] text-white rounded-2xl font-black text-xs uppercase tracking-widest">Back to Hotels</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20">
      ...
      <div className="bg-[#0A2540] text-white relative overflow-hidden">
         <div className="absolute top-0 right-0 w-[50%] h-full bg-gradient-to-l from-[#635BFF]/20 to-transparent" />
            ...
            <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-12">
               <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap items-center gap-6 text-sm font-medium text-white/60">
                     ...
                     <div className="flex items-center gap-2">
                        <BedDouble size={16} className="text-[#635BFF]" />
                        <span>{rooms.length} Room Types</span>
                     </div>
                     <div className="w-1.5 h-1.5 bg-white/10 rounded-full" />
                     <div className="flex items-center gap-2 text-white font-black text-xs uppercase tracking-widest">
                        <Star size={14} fill="#F6A100" className="text-[#F6A100]" />
               ...
               <div className="flex gap-4">
                  <button 
                    ...
                    className="flex items-center gap-3 bg-[#635BFF] text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-white hover:text-[#0A2540] transition-all shadow-2xl shadow-[#635BFF]/20 active:scale-95"
                  >
            ...
            <div className="flex border-b border-white/5">
               {['OVERVIEW', 'ROOMS', 'GALLERY'].map(tab => (
                  ...
                  >
                     {tab}
                     {activeTab === tab && (
                        <motion.div layoutId="tabLine" className="absolute bottom-0 left-0 right-0 h-1 bg-[#635BFF]" />
                     )}
                  </button>
               ))}
            </div>
               ...
               >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                     <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
                        <div className="w-12 h-12 rounded-2xl bg-blue-50 text-[#635BFF] flex items-center justify-center mb-4">
                           <TrendingUp size={24} />
                        </div>
                        <div>
                           <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Total Revenue</p>
                           <p className="text-4xl font-black text-[#0A2540] tracking-tighter">₹{stats?.totalRevenue?.toLocaleString() || '0'}</p>
                        </div>
                     </div>
                     <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
                        ...
                        <div>
                           ...
                           <p className="text-4xl font-black text-[#0A2540] tracking-tighter">{stats?.bookingCount || '0'}</p>
                        </div>
                     </div>
                     <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
                        ...
                        <div>
                           ...
                           <p className="text-4xl font-black text-[#0A2540] tracking-tighter">₹{Math.round(stats?.avgRevenue || 0).toLocaleString()}</p>
                        </div>
                     </div>
                  </div>
                     ...
                     <div className="lg:col-span-2 space-y-10">
                        <section className="space-y-6">
                           <div className="flex items-center gap-3">
                              ...
                              <h3 className="text-xs font-black text-[#0A2540] uppercase tracking-[0.3em]">Description</h3>
                           </div>
                        ...
                        <section className="bg-white rounded-[3rem] border border-gray-100 p-12 space-y-10">
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                              <div className="space-y-6">
                                 <h4 className="text-xs font-black text-[#635BFF] uppercase tracking-[0.3em]">Contact Information</h4>
                                 <div className="space-y-4">
                                    <div className="flex gap-4">
                                       <MapPin className="text-gray-300 shrink-0" size={18} />
                                       <p className="text-sm font-bold text-[#0A2540] tracking-tight">{hotel.contactInfo?.address || 'Physical deployment details not defined.'}</p>
                                    </div>
                                    <div className="flex gap-4">
                                       <Phone className="text-gray-300 shrink-0" size={18} />
                                       <p className="text-sm font-bold text-[#0A2540]">{hotel.contactInfo?.phoneNumber || 'Line inactive'}</p>
                                    </div>
                                    <div className="flex gap-4">
                                       <Mail className="text-gray-300 shrink-0" size={18} />
                                       <p className="text-sm font-bold text-[#0A2540]">{hotel.contactInfo?.email || 'nexus@system.io'}</p>
                                    </div>
                                 </div>
                              </div>
                              <div className="space-y-6">
                                 <h4 className="text-xs font-black text-[#635BFF] uppercase tracking-[0.3em]">Amenities</h4>
                     ...
                     {rooms.map((room, idx) => (
                        <div key={room.id} className="bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl transition-all group">
                           <div className="aspect-video relative overflow-hidden bg-gray-50">
                              ...
                              <div className="absolute top-6 left-6 px-4 py-1.5 bg-white/90 backdrop-blur-md rounded-xl text-xs font-black text-[#0A2540] uppercase tracking-widest border border-white/50">
                                 {room.type}
                              </div>
                           </div>
                           <div className="p-8 space-y-6">
                              <div className="flex items-center justify-between">
                                 ...
                                 <div className="text-right">
                                    ...
                                    <p className="text-xl font-black text-[#0A2540] tracking-tighter">{room.capacity} GUESTS</p>
                                 </div>
                              </div>
                              ...
                              <button 
                                ...
                                className="w-full py-4 bg-gray-50 text-[#0A2540] rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-[#0A2540] hover:text-white transition-all border border-gray-100"
                              >
                                View Availability
                              </button>
                           </div>
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 px-10 py-5 bg-[#0A2540]/90 backdrop-blur-2xl rounded-[2.5rem] border border-white/10 shadow-2xl z-50 flex items-center gap-10">
         ...
         <div className="flex items-center gap-6">
            <button className="text-xs font-black text-white/60 hover:text-[#635BFF] uppercase tracking-widest transition-all">Edit Hotel</button>
