...
const HotelSummaryPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const currentHotel =
        hotelRes.data?.data?.find((h) => h.id === parseInt(hotelId)) ||
      ...
      if (currentHotel) {
        ...
        const reportsRes = await adminHotelApi.getReports(hotelId, {
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center space-y-6">
        ...
        <button
          ...
          className="px-8 py-4 bg-[#0F172A] text-white rounded-2xl font-black text-xs uppercase tracking-widest"
        >
          Back to Hotels
        </button>
      </div>
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-20 pt-28">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-12">
            <div className="space-y-6">
              <div className="flex items-center gap-3">
                <span className="px-3 py-1 bg-white/10 text-white text-xs font-black uppercase tracking-widest rounded-md border border-white/10">
                  Hotel ID: #{hotel.id}
                </span>
                <div
                  className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest border ${hotel.active ? "bg-green-500/10 text-green-400 border-green-500/20" : "bg-red-500/10 text-red-400 border-red-500/20"}`}
                >
                  <div
                    className={`w-1 h-1 rounded-full ${hotel.active ? "bg-green-400" : "bg-red-400"}`}
                  />
            ...
            {["OVERVIEW", "ROOMS", "GALLERY"].map((tab) => (
              ...
              >
                ...
                {activeTab === tab && (
                  <motion.div
                    layoutId="tabLine"
                    className="absolute bottom-0 left-0 right-0 h-1 bg-[#0284C7]"
                  />
                )}
              </button>
            ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-4xl font-black text-[#0F172A] tracking-tighter">
                      ₹{Math.round(stats?.avgRevenue || 0).toLocaleString()}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="lg:col-span-2 space-y-10">
                  <section className="space-y-6">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-xs font-black text-[#0F172A] uppercase tracking-[0.3em]">
                        Description
                      </h3>
                    </div>
                    <p className="text-lg font-medium text-gray-500 leading-relaxed italic">
                      "{hotel.name} is a premium hotel within the Roomly
                      network, offering luxury and modern design."
                    </p>
                  </section>
                  ...
                  <section className="bg-white rounded-[3rem] border border-gray-100 p-12 space-y-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                      <div className="space-y-6">
                        <h4 className="text-xs font-black text-[#0284C7] uppercase tracking-[0.3em]">
                          Contact Information
                        </h4>
                        <div className="space-y-4">
                          <div className="flex gap-4">
                            ...
                            <p className="text-sm font-bold text-[#0F172A] tracking-tight">
                              {hotel.contactInfo?.address ||
                                "Physical deployment details not defined."}
                            </p>
                          </div>
                          <div className="flex gap-4">
                            <Phone
                          ...
                          <div className="flex gap-4">
                            ...
                            />
                            <p className="text-sm font-bold text-[#0F172A]">
                              {hotel.contactInfo?.email || "nexus@system.io"}
                            </p>
                          </div>
                        </div>
                      </div>
                      <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {hotel.amenities?.map((a) => (
                            <span
                              key={a}
                              className="px-3 py-1.5 bg-gray-50 text-xs font-black text-gray-400 uppercase tracking-widest rounded-lg border border-gray-100"
                            >
                              {a}
                            </span>
                          ))}
                    ...
                    <div className="p-8 space-y-6">
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-6 border-t border-gray-50">
                        {room.amenities?.slice(0, 4).map((a) => (
                          <span
                            key={a}
                            className="px-2 py-1 bg-gray-50 text-xs font-bold text-gray-400 rounded-md border border-gray-100"
                          >
                            {a}
                          </span>
                        ))}
                      ...
                      <button
                        onClick={() =>
                          navigate(
                            `/admin/hotels/${hotelId}/rooms/${room.id}/inventory`,
                            {
                              state: {
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 px-10 py-5 bg-[#0F172A]/90 backdrop-blur-2xl rounded-[2.5rem] border border-white/10 shadow-2xl z-50 flex items-center gap-10">
        <div className="flex items-center gap-4 border-r border-white/10 pr-10">
          <div
            className={`w-3 h-3 rounded-full ${hotel.active ? "bg-green-500 animate-pulse" : "bg-red-500"}`}
          />
        ...
        <div className="flex items-center gap-6">
          <button className="text-xs font-black text-white/60 hover:text-[#0284C7] uppercase tracking-widest transition-all">
            Edit Hotel
          </button>
          <button className="text-xs font-black text-red-400 hover:text-red-500 uppercase tracking-widest transition-all">
            Delete Hotel
          </button>
        </div>
      </div>
