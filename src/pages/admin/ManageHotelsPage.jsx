...
const EmptyState = ({ onAdd }) => (
  <div className="flex flex-col items-center justify-center py-32 text-center px-4">
    <div className="w-24 h-24 bg-[#635BFF]/5 rounded-[2.5rem] flex items-center justify-center mb-8 border border-[#635BFF]/10">
      <Building2 size={40} className="text-[#635BFF]" />
    </div>
    <h3 className="text-2xl font-black text-[#0A2540] tracking-tighter mb-2 uppercase">No Hotels Found</h3>
    ...
    <button
      onClick={onAdd}
      className="flex items-center gap-3 bg-[#0A2540] text-white px-10 py-4 rounded-[1.5rem] font-black text-xs uppercase tracking-widest hover:bg-[#635BFF] transition-all shadow-2xl shadow-[#0A2540]/20 active:scale-95"
    >
      <Plus size={18} />
    ...
    <div className="min-h-screen bg-[#F8FAFC] relative">
      ...
      <div className="max-w-screen-2xl mx-auto px-6 py-16 space-y-12">
        ...
        <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-10">
          <div className="space-y-6">
             <div className="flex items-center gap-3">
                <span className="px-3 py-1 bg-[#635BFF]/10 text-[#635BFF] text-xs font-black uppercase tracking-[0.3em] rounded-md border border-[#635BFF]/10">Admin Portal</span>
                <div className="h-1 w-1 bg-gray-300 rounded-full" />
                <span className="text-xs font-black text-gray-400 uppercase tracking-[0.3em]">Property Manager</span>
             </div>
             <h1 className="text-6xl font-black text-[#0A2540] tracking-tighter uppercase leading-none">Your Properties</h1>
             <p className="text-[#635BFF] font-black text-xs uppercase tracking-[0.4em]">Management interface for your hotels, room availability, and performance reports.</p>
          </div>
          
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex bg-white p-1 rounded-2xl border border-gray-100 shadow-sm">
                <button onClick={() => setViewMode('list')} className={`p-2 rounded-xl transition-all ${viewMode === 'list' ? 'bg-[#0A2540] text-white shadow-lg' : 'text-gray-400 hover:bg-gray-50'}`}><List size={18} /></button>
                <button onClick={() => setViewMode('grid')} className={`p-2 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-[#0A2540] text-white shadow-lg' : 'text-gray-400 hover:bg-gray-50'}`}><LayoutGrid size={18} /></button>
            </div>
            
            <button
              onClick={fetchHotels}
              disabled={isLoading}
              className="p-3.5 bg-white border border-gray-100 rounded-2xl text-gray-400 hover:text-[#0A2540] transition-all shadow-sm hover:shadow-md active:rotate-180"
            >
            ...
            <button
              ...
              className="flex items-center gap-3 bg-[#0A2540] text-white px-8 py-4 rounded-[1.5rem] font-black text-xs uppercase tracking-widest hover:bg-[#635BFF] cursor-pointer transition-all shadow-xl shadow-[#0A2540]/20 active:scale-95"
            >
              <Plus size={18} />
              <span>Register Property</span>
            </button>
                    ...
                    >
                       ...
                       <div className="col-span-3">
                          <h4 
                            ...
                            className="text-sm font-black text-[#0A2540] uppercase tracking-tighter mb-0.5 hover:text-[#635BFF] cursor-pointer transition-colors"
                          >
                            {hotel.name}
                          </h4>
                       ...
                       <div className="col-span-4 flex items-center justify-end gap-2">
                          <button onClick={() => navigate(`/admin/hotels/${hotel.id}/rooms`, { state: { hotelName: hotel.name } })} className="p-2.5 rounded-xl bg-[#635BFF]/5 text-[#635BFF] hover:bg-[#635BFF] hover:text-white transition-all group/btn" title="Manage Rooms">
                             <BedDouble size={18} />
                          </button>
                          ...
                          <button onClick={() => openEdit(hotel)} className="p-2.5 rounded-xl hover:bg-gray-100 text-gray-400 hover:text-[#0A2540] transition-all">
                             <Edit2 size={16} />
                          </button>
                ...
                >
                  <div className="aspect-video rounded-[1.5rem] bg-gray-50 overflow-hidden mb-6 relative">
                     ...
                     <div className="absolute top-4 right-4 flex gap-2">
                        <button onClick={() => openEdit(hotel)} className="p-2.5 rounded-xl bg-white/20 backdrop-blur-xl text-white border border-white/20 hover:bg-white hover:text-[#0A2540] transition-all"><Edit2 size={14} /></button>
                     </div>
                     {!hotel.active && <div className="absolute inset-0 bg-[#0A2540]/60 backdrop-blur-sm flex items-center justify-center font-black text-white text-xs uppercase tracking-[0.3em]">OFFLINE</div>}
                  </div>
                  ...
                  <div className="space-y-4">
                     <div>
                        <div className="flex items-center justify-between mb-1">
                           <h4 
                             onClick={() => navigate(`/admin/hotels/${hotel.id}`)}
                             className="text-lg font-black text-[#0A2540] uppercase tracking-tighter truncate hover:text-[#635BFF] cursor-pointer transition-colors"
                           >
                             {hotel.name}
                           </h4>
                        ...
                        <button onClick={() => navigate(`/admin/hotels/${hotel.id}/rooms`)} className="flex items-center justify-center gap-2 py-3 bg-[#0A2540] text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-[#635BFF] transition-all">Rooms</button>
                        <button onClick={() => navigate(`/admin/reports?hotelId=${hotel.id}`)} className="flex items-center justify-center gap-2 py-3 bg-gray-50 text-[#0A2540] rounded-xl text-xs font-black uppercase tracking-widest hover:bg-gray-100 transition-all">Reports</button>
                     </div>
                  </div>
                </motion.div>
              ))}
        ...
        {!isLoading && hotels.length > 0 && (
          <div className="bg-[#0A2540] rounded-[2rem] p-8 text-white flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
             ...
             <div className="flex items-center gap-10">
              ...
              <button 
                ...
                className="flex items-center gap-3 bg-white text-[#0A2540] px-8 py-3.5 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-[#635BFF] hover:text-white cursor-pointer transition-all active:scale-95 shadow-xl shadow-white/5"
              >
